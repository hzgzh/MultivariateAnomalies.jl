...
function dist_matrix!(D_out::Tuple{Array{tp,2},Array{tp,2},Array{tp,2}}, data::AbstractArray{tp, N}; dist::String = "Euclidean", space::Int = 0, lat::Int = 0, lon::Int = 0, Q = 0, dims = 2) where {tp, N}
  ...
  transpose!(tdat, dat)
  if(dist == "Euclidean")         Distances.pairwise!(D, Euclidean(), tdat, dims = dims)
  elseif(dist == "SqEuclidean")   Distances.pairwise!(D, SqEuclidean(), tdat, dims = dims)
  elseif(dist == "Chebyshev")     Distances.pairwise!(D, Chebyshev(), tdat, dims = dims)
  elseif(dist == "Cityblock")     Distances.pairwise!(D, Cityblock(), tdat, dims = dims)
  elseif(dist == "JSDivergence")  Distances.pairwise!(D, JSDivergence(), tdat, dims = dims)
  elseif(dist == "Mahalanobis")   Distances.pairwise!(D, Mahalanobis(Q), tdat, dims = dims)
  elseif(dist == "SqMahalanobis") Distances.pairwise!(D, SqMahalanobis(Q), tdat, dims = dims)
...
function dist_matrix!(D_out::Tuple{Array{tp,2},Array{tp,2},Array{tp,2},Array{tp,2},Array{tp,2}},
  ...
  if(dist == "Euclidean")         Distances.pairwise!(D, Euclidean(), ttraindat, tdat, dims = dims)
  elseif(dist == "SqEuclidean")   Distances.pairwise!(D, SqEuclidean(), ttraindat, tdat, dims = dims)
  elseif(dist == "Chebyshev")     Distances.pairwise!(D, Chebyshev(), ttraindat, tdat, dims = dims)
  elseif(dist == "Cityblock")     Distances.pairwise!(D, Cityblock(), ttraindat, tdat, dims = dims)
  elseif(dist == "JSDivergence")  Distances.pairwise!(D, JSDivergence(), ttraindat, tdat, dims = dims)
  elseif(dist == "Mahalanobis")   Distances.pairwise!(D, Mahalanobis(Q), ttraindat, tdat, dims = dims)
  elseif(dist == "SqMahalanobis") Distances.pairwise!(D, SqMahalanobis(Q), ttraindat, tdat, dims = dims)
  else print("$dist is not a defined distance metric, has to be one of 'Euclidean', 'SqEuclidean', 'Chebyshev', 'Cityblock' or 'JSDivergence'")
  end
  return(D_out[1])
end
